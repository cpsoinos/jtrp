describe Item::Updater do
  ...
  it "can be instantiated" do
    expect(Item::Updater.new(item)).to be_an_instance_of(Item::Updater)
  end

  it "updates an item" do
    Item::Updater.new(item).update(attrs)
  ...
  context "clover" do
    ...
    it "syncs to clover when inventory item already exists" do
      Item::Updater.new(active_item).update(attrs)
    ...
    it "syncs to clover when inventory item does not exist" do
      ...
      Item::Updater.new(other_item).update(attrs)
  ...
  it "processes a sale" do
    Item::Updater.new(active_item).update(sale_price: 5000)

    expect(active_item).to be_sold
  end

  it "processes sale date" do
    Item::Updater.new(active_item).update(sold_at: "5/4/16", sale_price: 5000)

...
class LabelGenerator
  ...
  def generate
    Prawn::Labels.render(@items, type: "Burris8UP") do |pdf, item|
      ...
      pdf.text_box "SKU:\nItem No.:\n#{show_client_name?(item) ? item.account.client.last_name : 'JTRP No.:'}", at: [0, 85], height: 75, width: 80, size: 10
  ...
  def show_client_name?(item)
    item.client_intention == "consign" && !item.expired?
  end
